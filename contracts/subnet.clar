;; title: hooter-the-owl-subnet
;; author: rozar.btc
;; summary: Intent-based subnet token wrapper for hooter-the-owl using blaze-intent verifier.
...
(define-read-only (get-name)         (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.hooter-the-owl get-name))
(define-read-only (get-symbol)       (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.hooter-the-owl get-symbol))
(define-read-only (get-decimals)     (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.hooter-the-owl get-decimals))
(define-read-only (get-token-uri)    (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.hooter-the-owl get-token-uri))
(define-read-only (get-total-supply) (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.hooter-the-owl get-total-supply))
...
(define-public (withdraw (amount uint) (recipient (optional principal)))
  (let ((owner tx-sender)
    ...
    (try! (as-contract (contract-call? 'SP2ZNGJ85ENDY6QRHQ5P2D4FXKGZWCKTB2T0Z55KS.hooter-the-owl transfer amount tx-sender recv none)))
